		...
		while(true)
		{
			try
			{
				...
				if (s.empty())
				{
						...
						if (r==0)
						next=(String)nodeD.df.getValueAt(0,0);
						else
						next=(String)nodeD.df.getValueAt(1,0);
				}		
				else
				{
				...
				if (next.equals("A"))
				nodeD.df.setValueAt(cost.pop(),0,1);
				else
				if (next.equals("C"))
				nodeD.df.setValueAt(cost.pop(),1,1);
